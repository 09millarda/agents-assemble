...
Qualification targets include five concurrent collaborators, two runs, a 100-node graph, 50 KiB Markdown, p95 connected editing/output propagation within two seconds under the recorded network conditions, and reconnect convergence with no lost acknowledged edits. These are acceptance requirements, not achieved measurements. The contract pins a reference Linux/Codex baseline subject to integrated qualification. Decision #12 settles the bounded collaboration revision protocol; #13–#16 address native conversation, community publication/moderation, Lambda deployment recovery and full semantic graph collaboration. License/parity #5, detailed access roles, billing seat rules, packaging and hosting remain open.
