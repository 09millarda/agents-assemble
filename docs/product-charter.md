...
Qualification targets include five concurrent collaborators, two runs, a 100-node graph, 50 KiB Markdown, p95 connected editing/output propagation within two seconds under the recorded network conditions, and reconnect convergence with no lost acknowledged edits. These are acceptance requirements, not achieved measurements. The contract pins a reference Linux/Codex baseline subject to integrated qualification. Decisions #12–#15 address collaboration revisions, native conversation, community publication/moderation and Lambda deployment recovery. License/parity #5, detailed access roles, billing seat rules, packaging and hosting remain open.
