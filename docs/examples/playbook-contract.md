...
| `ReviewResult` | `accepted: boolean`, exact spec reference, reviewed checkpoint, findings artifact; acceptance is for this exact commit and spec. |
| `CheckResult` | `passed: boolean`, exact spec reference, checked checkpoint, evidence artifact with commands/results. |
...
Decision #6's [behavioral experiment](../research/playbook-conformance.md) materializes these examples in a separate disposable archive. Its two explicit mock retry classes do not change the default here or certify real adapters. Successor admission requires a new bounded work grant; an edit cannot silently reset the predecessor's allowance. Implementation and publication validate canonical accepted receipts, with verification bound to both the exact spec and checkpoint.
