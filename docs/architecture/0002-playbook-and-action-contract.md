...
Resolved [prototype decision #6](https://github.com/09millarda/agents-assemble/issues/6): retain the grammar with the amendments above. The next sharp decision is the durable execution substrate and context-owned recovery protocol, using the experiment's traces as acceptance examples. Compare candidates and prove failure behavior before selecting a production framework.
