...
Local/self-hosted catalogs require local access policy, never a hosted invitation or WorkOS account. Contacting the hosted public service subjects public contributions to that service's identity and moderation rules. This ADR originally left license/parity to #5; [ADR 0011](0011-open-source-licensing-and-edition-parity.md) now records Apache-2.0, full self-hosted parity and open-source public software package admission. Actual license application remains separate.
...
The sharp follow-up is [#17, portable envelope and verifier conformance](https://github.com/09millarda/agents-assemble/issues/17): canonical bytes, component inventory, parser/resource limits, origin verification and safe import/export across two installations. Broader discovery ranking, abuse operations, retention, private federated registries, ownership transfers and production service implementation remain fog until their requirements are sharp. License/parity is resolved by ADR 0011/#5, with license application and distribution review still pending; runner observer trust remains #11; deployment recovery remains #15.

## Subsequent envelope decision

[ADR 0012](0012-portable-package-envelope-and-verification.md)/[#17](https://github.com/09millarda/agents-assemble/issues/17) resolves the bounded format, identity, parser, offline trust and public ancestry contract above. Actual byte/signature/SQLite probes support a strict stored-ZIP profile; full semantic validation, durable source allocation, production publisher authorization, legal permission review and integrated execution policy remain unproved.
