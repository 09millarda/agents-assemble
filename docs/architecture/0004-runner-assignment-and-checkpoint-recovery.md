...
First-release scope #4 is subsequently resolved in the [release contract](../first-release-contract.md), selecting Linux/Codex without turning this experiment into release certification. License/parity (#5), authenticated tunnel/receipt provenance at this decision boundary, live integration publication, enrollment, secrets, packaging and broader harness/OS compatibility remain further work. The observed surviving native tool makes [#9, supervision and scoped stopped-writer evidence](https://github.com/09millarda/agents-assemble/issues/9), the next sharp technical decision. The subsequent [ADR 0005](0005-native-writer-supervision.md) resolves #9 with a conditional retained-payload supervision profile and exact stop-receipt rules. Uncertain scope, incomplete writer coverage or untrusted provenance continue to require a recovery pause.
