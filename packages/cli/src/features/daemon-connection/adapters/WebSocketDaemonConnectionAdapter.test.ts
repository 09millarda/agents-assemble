...
async function connectDaemon(workflow: WorkflowConnectionHandler) {
  ...
  socket.emit("open");
  expect(JSON.parse(FakeDaemonSocket.sent[0]!)).toEqual({
    type: "daemon.hello",
    daemonId: "daemon-1",
...
test("the authenticated workflow gateway preserves command identities, worktree and immutable document bindings", async () => {
  ...
  const socket = FakeDaemonSocket.instances[0]!;
  socket.emit("open");
  expect(sentFrames()).toContainEqual({ type: "daemon.hello", daemonId: "daemon-1", machineName: "workstation", capabilities });
