...
def controller(config):
    ...
    try:
        ...
        denial_config = {**public, **{k: config[k] for k in ["socket", "binding_digest", "keeper_pid", "payload", "key", "journal"]}}
        ...
        denials = json.loads((workspace / "denials.json").read_text())
        ...
        native_state = json.loads((workspace / "native-status.json").read_text())
        marker = json.loads((workspace / "started.json").read_text())
        members = []
        ...
        size = (workspace / "ticks.txt").stat().st_size; time.sleep(.3)
        assert (workspace / "ticks.txt").stat().st_size > size
