...
Date: 2026-09-12. Constraints below come from the user's brief. Bounded contexts remain a provisional architecture sketch. ADR 0001 settles the control-plane/runner boundary; ADR 0002 settles the playbook vocabulary and definition/execution baseline, retained with #6 amendments after a bounded behavioral simulation. Production durability and adapter conformance remain to validate. Consult the [canonical map](https://github.com/09millarda/agents-assemble/issues/1) and [product charter](docs/product-charter.md).
...
Approval binds an exact operation/artifact/code manifest. Relevant specification edits default to pausing after observation and explicit retain/adopt review. Adoption uses a linked successor run starting at entry with verified checkpoint/artifact inputs and inherited external-resource mappings. Completed history is not rewritten. These edit/adaptation defaults were selected during wayfinding without an owner response and remain amendable. Execution substrate, schema/wire implementation and actual adapter conformance are still undecided. See the [examples](docs/examples/playbook-contract.md).

## Specification readiness

Decision #6's [conformance report](docs/research/playbook-conformance.md) retains the grammar and makes ordered source edit observations, canonical scope/publication receipts, independent recovery obligations and explicitly admitted successor budgets concrete. The mock model passes 25 authoring probes and 25 runtime scenarios; it does not prove context-local database/outbox transactions, durable timers, leases or real harness behavior. The next technical decision is the durable execution substrate and recovery protocol.

Before implementation tickets, validate the definition/execution contract, settle the first release boundary and license/parity policy, and prove the smallest harness-to-human-to-checkpoint journey. Later decisions must include actual aggregate invariants, command/event schemas, compatibility policy, testable recovery guarantees, and examples of concurrent human/agent behavior. This document preserves the destination while those details remain open.
