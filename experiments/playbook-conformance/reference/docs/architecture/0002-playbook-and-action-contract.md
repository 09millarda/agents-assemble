...
Date: 2026-09-12  
Status: **Accepted architecture baseline for #3; conformance remains to be prototyped.**  
Decision: [#3](https://github.com/09millarda/agents-assemble/issues/3) · Map: [#1](https://github.com/09millarda/agents-assemble/issues/1)  
...
These semantics require durable state, timers, idempotent messages, bounded scheduling, logical effect records and reconciliation. A queue plus ephemeral callbacks is insufficient. A context-owned PostgreSQL state machine with workers, a durable workflow runtime behind a port, and a compatible external workflow interpreter are candidates to compare later. None is selected here. Whatever substrate is chosen must preserve context-local outbox/inbox transactions and saga ownership from ADR 0001; a global workflow transaction may not write multiple contexts' tables.
...
## Sharp next experiment and remaining fog

Graduated [prototype decision #6](https://github.com/09millarda/agents-assemble/issues/6): can a small, disposable conformance model preserve a definition through builder/document/editor operations and preserve occurrence, approval and effect identity across restart/retry/edit races? Use these two examples, a mock runner and a mock publication adapter. Include an oversized/duplicate-key fan-out rejection and a successor run adopting an edited spec. Its verdict should retain or revise this grammar and identify durable-substrate requirements, without selecting production frameworks by habit.

Remaining fog: production schemas and adapter/wire versions; canonical bytes and signing; runtime/framework selection; detailed saga protocols and recovery UI; selective replay; nested playbook recursion/composition beyond fixed action reuse; large-scale scheduling; actual harness conformance; runner isolation; editor technology and package registry governance. First release and license/parity remain #4 and #5.

Research inputs: [authoring](../research/playbook-authoring.md) and [execution failure review](../research/playbook-execution-review.md). Their recommendations are evidence, not additional resolved tickets. Validation performed: two independent research/review passes, correction of example/grammar inconsistencies, parsing all three illustrative YAML blocks, and resolving local Markdown links. No schema conformance, interpreter, harness or recovery experiment has been run.
