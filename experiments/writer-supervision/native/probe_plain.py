...
def props(unit):
    names = ['Id','LoadState','ActiveState','SubState','MainPID','ControlGroup','InvocationID',
             'Type','ExitType','Restart','RestartForceExitStatus','KillMode','TimeoutStopUSec','SendSIGKILL','Result','NRestarts','RuntimeMaxUSec']
...
def main():
    ...
    data={'mode':args.mode,'status':'started','version':command(['codex','--version']).stdout.strip(),
          ...
          'workspace':str(repo),'unit':unit,'nativeTurnStarts':0,
    ...
    def launch(env):
        return ['systemd-run','--user','--quiet','--pipe','--wait','--unit='+unit,
                ...
                '--property=TimeoutStopSec=2s','--property=SendSIGKILL=yes','--property=RuntimeMaxSec=240s',
                '--property=WorkingDirectory='+str(repo),'/usr/bin/env','-i'] + [k+'='+v for k,v in env.items()] + [shutil.which('codex'),'app-server','--stdio']
    try:
        ...
        if args.mode=='graceful':
            ...
            main_fd=os.pidfd_open(int(data['before']['MainPID']))
