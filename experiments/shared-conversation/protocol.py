...
class Lab:
 def __init__(self,path):
  ...
  self.service=Journal(self.path/'service.db');self.daemon=Journal(self.path/'daemon.db');self.native=Journal(self.path/'native.db');self.dispatch_claims=set()
 def close(self):
 ...
 def submit(self,key='input-1',sender='alice',text='ALPHA',scope=None,kind='steer'):
  def f(s):
   ...
   limit=1 if kind=='interrupt' else 4
   if len([c for c in s['commands'].values() if c['status']=='queued' and c['body']['kind']==kind])>=limit:return 'queue-full'
 ...
 def gate(self,key):
  ...
  if b['kind']=='steer' and any(x['body']['kind']=='interrupt' and d['commands'].get(k,x)['status']!='rejected' for k,x in s['commands'].items()):return 'interrupt-barrier'
  ...
  if b['kind']=='steer' and any(x['seq']<c['seq'] and d['commands'].get(k,{}).get('status') not in ('accepted','rejected') for k,x in s['commands'].items()):return 'ordered-wait'
  return 'allowed'
 def intent(self,key):
  g=self.gate(key)
  ...
  def f(d):
   c=d['commands'][key]
   if c['status']!='received':return 'inflight' if c['status']=='sending' else c['status']
   c['status']='sending';return 'dispatch-ready'
  r=self.daemon.change(f)
  if r=='dispatch-ready':self.dispatch_claims.add(key)
  return r
 def native_call(self,key,outcome='accepted'):
  # No native dedup assumed; count actual modeled side effects.
  if key not in self.dispatch_claims:return 'no-dispatch-claim'
  self.dispatch_claims.remove(key)
 ...
 def reconcile(self,key):
  ...
  def f(s):
   ...
   if old['status'] in ('accepted','rejected'):return old['status'] if c['status']==old['status'] else 'stale-or-conflicting-receipt'
 ...
 def recover(self):
  ...
  self.daemon.change(f);self.dispatch_claims.clear()
 def send(self,key):
  self.receive(key);r=self.intent(key)
  if r=='dispatch-ready':self.native_call(key);self.receipt(key);return self.reconcile(key)
  return r
 def output(self,seq,payload,epoch='journal-A'):
  def f(s):
   ...
   if seq<=s['cursor']:return 'expired'
   if seq!=s['cursor']+1:return 'gap'
   s['output'][key]={'digest':h,'payload':payload};s['cursor']=seq
   s['output']={k:v for k,v in s['output'].items() if int(k.split(':')[-1])>seq-3}
   return 'stored'
  return self.service.change(f)
 def replay(self,sender,cursor):
  ...
  return {'through':s['cursor'],'events':[{'seq':int(k.split(':')[-1]),**v} for k,v in sorted(s['output'].items(),key=lambda kv:int(kv[0].split(':')[-1])) if int(k.split(':')[-1])>cursor]}
